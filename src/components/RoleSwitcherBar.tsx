    ...
    <div id="role-switcher-container" className="bg-white border-b border-slate-200 shadow-xs">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2 flex flex-col md:flex-row md:items-center md:justify-between gap-2.5">
        ...
        <div className="flex items-center space-x-2 shrink-0">
          {isAuthenticated ? (
            <div className="inline-flex items-center space-x-2 px-3 py-1.5 rounded-xl bg-slate-900 text-white text-xs shadow-xs border border-slate-800">
              <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
              <div className="flex items-center space-x-1.5 font-medium">
                <span className="text-slate-400 text-[11px]">Perfil Ativo:</span>
                <span className="font-bold text-amber-400">{getRoleNamePt(currentUser.role)}</span>
          ...
          {onOpenSecurityAuditor && currentUser.role === 'admin' && (
            <button
              ...
              className="inline-flex items-center space-x-1.5 px-2.5 py-1.5 rounded-xl bg-amber-50 hover:bg-amber-100 text-slate-900 text-[11px] font-bold border border-amber-300 transition cursor-pointer shadow-2xs"
        ...
        <div className="flex items-center space-x-2 overflow-x-auto no-scrollbar py-0.5">
          {visibleTabs.map((tab) => {
            ...
            return (
              ...
              >
                <div className={`p-1 rounded-lg ${isActive ? 'bg-black text-amber-400' : 'bg-amber-50 text-amber-600 border border-amber-200/60'}`}>
                  ...
                  <div className="flex items-center space-x-1.5">
                    ...
                    {tab.badge && (
                      <span className={`text-[9px] font-black px-1.5 py-0.2 rounded whitespace-nowrap ${
                        isActive ? 'bg-black text-amber-400' : 'bg-[#cf102d] text-white'
                      }`}>
                        {tab.badge}
                      </span>
                    )}
