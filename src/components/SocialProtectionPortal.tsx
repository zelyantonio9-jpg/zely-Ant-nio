...
import { FormalizationTrackerHub } from './FormalizationTrackerHub';
import { FormalizationDiagnosisModal } from './FormalizationDiagnosisModal';
...
export const SocialProtectionPortal: React.FC = () => {
  ...
  const [activeSubTab, setActiveSubTab] = useState<'formalizacao' | 'consulta_inss' | 'simulador' | 'auditoria'>('formalizacao');
  ...
  return (
    <div id="social-protection-portal" className="space-y-5 pb-12">
      ...
      <div className="flex items-center space-x-2 border-b border-slate-200 pb-2">
        <button
          type="button"
          onClick={() => setActiveSubTab('formalizacao')}
          className={`px-4 py-2 rounded-lg text-xs font-bold transition flex items-center space-x-2 cursor-pointer ${
            activeSubTab === 'formalizacao'
              ? 'bg-[#FF6B00] text-white shadow-xs'
              : 'bg-white text-slate-700 hover:bg-slate-100 border border-slate-200'
          }`}
        >
          <Sparkles className="w-3.5 h-3.5" />
          <span>Roteiro de Formalização (PREI)</span>
        </button>

        <button
          type="button"
          onClick={() => setActiveSubTab('consulta_inss')}
          className={`px-4 py-2 rounded-lg text-xs font-bold transition flex items-center space-x-2 cursor-pointer ${
            activeSubTab === 'consulta_inss'
              ? 'bg-slate-900 text-white shadow-xs'
              : 'bg-white text-slate-700 hover:bg-slate-100 border border-slate-200'
          }`}
        >
          <Search className="w-3.5 h-3.5" />
          <span>Consulta Oficial INSS (Read-Only)</span>
        </button>
        ...
        >
          ...
          <span>Simulador de Benefícios</span>
        </button>
        ...
        >
          ...
          <span>Logs de Auditoria ({inssAuditLogs.length})</span>
        </button>
      </div>

      {/* TAB 1: FORMALIZATION TRACKER */}
      {activeSubTab === 'formalizacao' && (
        <FormalizationTrackerHub
          currentUser={currentUser}
          dossier={currentDossier}
          stages={formalizationStages.filter(s => s.dossierId === currentDossier?.id)}
          documents={formalizationDocuments.filter(d => d.dossierId === currentDossier?.id)}
          referrals={institutionalReferrals.filter(r => r.dossierId === currentDossier?.id)}
          inssVerification={inssVerifications.find(v => v.dossierId === currentDossier?.id)}
          auditLogs={formalizationAuditLogs.filter(a => a.dossierId === currentDossier?.id)}
          onOpenDiagnosis={() => setIsDiagnosisModalOpen(true)}
          onUploadDocument={uploadFormalizationDoc}
          onGenerateReferral={generateInstitutionalReferral}
          onRefreshStatus={() => {}}
        />
      )}

      {/* TAB 2: CONSULTA OFICIAL INSS */}
      {activeSubTab === 'consulta_inss' && (
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
            ...
            {validationResult && (
              <div className="mt-4 pt-4 border-t border-slate-100 space-y-3">
                <div className={`p-4 rounded-xl border flex items-start justify-between ${
                  validationResult.isValid && validationResult.complianceStatus === 'REGULAR'
                    ? 'bg-emerald-50/70 border-emerald-300'
                    : validationResult.isValid && validationResult.complianceStatus === 'IRREGULAR'
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="text-slate-600 space-y-0.5 font-mono text-[11px]">
                      <div>NISS: <strong>{validationResult.niss}</strong> | NIF: <strong>{validationResult.nif}</strong></div>
                      <div>Regime: <strong>{validationResult.regimeType}</strong> ({validationResult.activitySector})</div>
                      <div>Última Contribuição: <strong>{validationResult.lastContributionMonth || 'Não registada'}</strong></div>
                      {validationResult.certificateNumber && (
                        <div>Certidão: <strong>{validationResult.certificateNumber}</strong> (Válida até {validationResult.certificateExpiryDate})</div>
                      )}
                    </div>
                  </div>
                </div>
                ...
                {validationResult.isValid && (
                  <div className="p-3.5 bg-slate-50 border border-slate-200 rounded-xl space-y-3">
                    <label className="flex items-start space-x-2 text-slate-700 text-xs cursor-pointer">
                      <input
                        type="checkbox"
                        checked={userConsent}
                      ...
                      />
                      <span>
                        Autorizo expressamente o AO MARKET a sincronizar estes dados do INSS para efeitos de atribuição do Selo de Entidade Verificada e conformidade institucional.
                      </span>
                    </label>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {isLinking ? (
                          ...
                          <>
                            ...
                            <span>Vincular ao Perfil AO MARKET</span>
                          </>
                        )}
                      </button>
                      ...
                      {validationResult.certificateNumber && (
                        <a
                          href={`https://inss.gov.ao/certidoes/${validationResult.certificateNumber}`}
                          target="_blank"
                          rel="noreferrer"
                          className="px-3 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold rounded-xl text-xs flex items-center space-x-1 transition-colors"
                        >
        ...
        {/* Right Column: Verified Badge Digital Card */}
      ...
      {/* TAB 3: SIMULADOR DE BENEFÍCIOS */}
      {activeSubTab === 'simulador' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
          <div className="lg:col-span-7 bg-white rounded-2xl border border-slate-200 p-6 space-y-4 text-xs shadow-xs">
            ...
            <div className="space-y-4">
              <div>
                <label className="text-slate-700 block mb-1.5 font-bold text-xs">
                  Volume Mensal de Faturação Estimado / Transacionado (Kz):
                </label>
                <input
                  type="number"
                  step="10000"
                  ...
                  className="w-full bg-slate-50 border border-slate-300 text-slate-900 rounded-xl px-4 py-2.5 text-sm font-mono font-bold focus:outline-none focus:border-amber-500 focus:bg-white"
                />
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-1">
                  <div className="text-xs text-slate-600 font-semibold">Contribuição Mensal ({currentUser.role === 'producer' ? '3%' : '4%'}):</div>
                  <div className="text-xl font-black text-slate-900 font-mono">{formatKz(calculatedINSS)}</div>
                  <div className="text-[10px] text-slate-500">Dedução voluntária por fatura</div>
                </div>

                <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-200 space-y-1">
                  <div className="text-xs text-emerald-800 font-semibold">Pensão Vitalícia Estimada:</div>
                  <div className="text-xl font-black text-emerald-900 font-mono">{formatKz(estimatedPensionAOA)}</div>
                  <div className="text-[10px] text-emerald-700 font-medium">Garantia pelo Estado de Angola</div>
                </div>
              </div>
            </div>
          </div>

          <div className="lg:col-span-5 bg-white rounded-2xl border border-slate-200 p-6 space-y-3 text-xs shadow-xs">
            ...
            <div className="space-y-2.5 text-slate-700">
              <div className="flex items-start space-x-2">
                ...
                <span>Segurança em caso de doença, maternidade ou invalidez profissional.</span>
              </div>
              <div className="flex items-start space-x-2">
                <Check className="w-4 h-4 text-emerald-600 shrink-0 mt-0.5" />
                <span>Pensão de velhice com garantia do Instituto Nacional de Segurança Social.</span>
              </div>
              <div className="flex items-start space-x-2">
                <Check className="w-4 h-4 text-emerald-600 shrink-0 mt-0.5" />
                <span>Prioridade no acesso a subsídios agrários e microcrédito do PREI.</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <FormalizationDiagnosisModal
        ...
        onComplete={async (input, result) => {
          await startOrUpdateFormalization(input, result);
          setIsDiagnosisModalOpen(false);
          setActiveSubTab('formalizacao');
        }}
      />
    </div>
  );
